...
- Probar arquitecturas que separan KMS, wallet y DID en servicios dedicados
- Facilitar el escalado y la operacion en produccion

## Arquitectura

Servicios agnosticos (KMS, wallet, DID) y agentes Credo se comunican de forma independiente:

```
  +-----------------+   +-----------------+   +-----------------+
  |  kms-service    |   | wallet-service  |   |  vdr-service    |
  |  :4001          |   |  :4002          |   |  :4003          |
...
| **wallet-service**  | 4002 | Almacenamiento de wallet por (type, id). Credo persiste DidRecord, ConnectionRecord, OutOfBandRecord, etc. |
...
curl http://localhost:4002/health  # wallet
...
- `KMS_MODE` / `EXTERNAL_KMS_URL`
- `WALLET_MODE` / `EXTERNAL_WALLET_URL`
- `VDR_SERVICE_URL`
...
- NestJS - API REST
- SQLite - Persistencia en kms, wallet y vdr
- Docker - Orquestacion
