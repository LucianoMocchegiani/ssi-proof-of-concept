...
export class InternalKeyManagementService implements Kms.KeyManagementService {
  ...
  private getKey(keyId: string): StoredKeyPair | null {
    const row = this.db
      .prepare('SELECT publicJwk, privateJwk FROM keys WHERE id = ?')
  ...
  private saveKey(
    ...
    this.db
      .prepare('INSERT OR REPLACE INTO keys (id, publicJwk, privateJwk) VALUES (?, ?, ?)')
      .run(keyId, JSON.stringify(publicJwk), JSON.stringify(privateJwk))
  }
  ...
  public async createKey<Type extends Kms.KmsCreateKeyType>(
    ...
    if (type.kty !== 'OKP' || type.crv !== 'Ed25519') {
      throw new Error(
        'Only OKP Ed25519 supported in InternalKeyManagementService'
      )
    }
  ...
  public async importKey<Jwk extends Kms.KmsJwkPrivate>(
    ...
    const publicJwk = Kms.publicJwkFromPrivateJwk(
      options.privateJwk as Kms.KmsJwkPrivate
    )
    publicJwk.kid = kid
    this.saveKey(
      kid,
      publicJwk as unknown as Ed25519PublicJwk,
      options.privateJwk as unknown as Ed25519PrivateJwk
    )
  ...
  public async deleteKey(
    ...
    const result = this.db
      .prepare('DELETE FROM keys WHERE id = ?')
      .run(options.keyId)
  ...
  public async sign(
    ...
    if (algorithm !== 'EdDSA' && algorithm !== 'Ed25519') {
      throw new Kms.KeyManagementError(
        `Only EdDSA/Ed25519 supported, got ${algorithm}`
      )
    }
    const data =
      options.data instanceof Uint8Array
        ? options.data
        : Buffer.from(options.data as unknown as ArrayBuffer)
    const privateKey = createPrivateKey({
      key: entry.privateJwk as unknown as JsonWebKey,
      format: 'jwk',
    })
  ...
  public async verify(
    ...
    if (!publicJwk)
      throw new Kms.KeyManagementError('Public key not found for verify')
    if (algorithm !== 'EdDSA' && algorithm !== 'Ed25519') {
      throw new Kms.KeyManagementError(
        `Only EdDSA/Ed25519 supported, got ${algorithm}`
      )
  ...
  private ed25519ToX25519Secret(keyId: string): Uint8Array {
    ...
    if (ed25519Secret.length !== 64)
      throw new Kms.KeyManagementError(
        'Invalid Ed25519 key format (expected 64 bytes)'
      )
    const x25519Secret = ed2curve.convertSecretKey(
      new Uint8Array(ed25519Secret)
    )
    if (!x25519Secret)
      throw new Kms.KeyManagementError(
  ...
  public async encrypt(
    ...
    const data =
      opts.data instanceof Uint8Array
        ? opts.data
        : Uint8Array.from(
            Buffer.from(opts.data as unknown as ArrayBuffer)
          )
    ...
    if (key?.keyAgreement) {
      ...
      const externalPublicJwk =
        ka.externalPublicJwk ?? ka.recipientPublicKey ?? ka.recipientKey
      if (!externalPublicJwk?.x)
        throw new Kms.KeyManagementError(
          'encrypt: keyAgreement.externalPublicJwk required'
        )
      const theirX25519Pub = Buffer.from(externalPublicJwk.x, 'base64url')
      if (theirX25519Pub.length !== 32)
        throw new Kms.KeyManagementError(
          'encrypt: externalPublicJwk.x must be 32 bytes'
        )
      ...
      if (ephemeralPub) {
        ...
        return {
          ...
          iv: undefined as unknown as Uint8ArrayBuffer,
    ...
    if (key?.privateJwk?.kty === 'oct') {
      ...
      if (symKey.length !== 32)
        throw new Kms.KeyManagementError(
          'encrypt: symmetric key must be 32 bytes'
        )
      const iv = randomBytes(12)
      const encOpts: EncryptionOptions = opts.encryption ?? {}
      const aad = encOpts.aad
        ? typeof encOpts.aad === 'string'
          ? Buffer.from(encOpts.aad, 'base64')
          : Buffer.from(encOpts.aad)
        : undefined
      const cipher = createCipheriv('chacha20-poly1305', symKey, iv, {
        authTagLength: 16,
      } as Parameters<typeof createCipheriv>[3])
      if (aad)
        (cipher as unknown as { setAAD(buf: Buffer): void }).setAAD(aad)
    ...
    throw new Kms.KeyManagementError(
      'encrypt: key.keyAgreement or key.privateJwk (oct) required'
    )
  }
  ...
  public async decrypt(
    ...
    if (key?.keyAgreement) {
      ...
      if (!keyId)
        throw new Kms.KeyManagementError(
          'decrypt: keyAgreement.keyId required'
        )
      const x25519Secret = this.ed25519ToX25519Secret(keyId)
      const externalPublicJwk = ka.externalPublicJwk

      const iv =
        dec.iv instanceof Uint8Array
          ? Buffer.from(dec.iv)
          : typeof dec.iv === 'string'
            ? Buffer.from(dec.iv, 'base64')
            : null
      ...
      if (externalPublicJwk?.x && iv && iv.length === 24) {
        ...
        if (!opened)
          throw new Kms.KeyManagementError(
            'decrypt: nacl.box.open failed (authcrypt)'
          )
      ...
      if (encrypted.length <= 56)
        throw new Kms.KeyManagementError(
          'decrypt: encrypted too short for anoncrypt (need 32+24+data)'
        )
      ...
      const opened = nacl.box.open(
        ...
        new Uint8Array(extractedNonce),
        new Uint8Array(ephemeralPub),
        x25519Secret
      )
      if (!opened)
        throw new Kms.KeyManagementError(
          'decrypt: nacl.box.open failed (anoncrypt)'
        )
      return { data: Buffer.from(opened) as unknown as Uint8ArrayBuffer }
    }
    ...
    if (key?.privateJwk?.kty === 'oct') {
      ...
      if (!iv || iv.length !== 12)
        throw new Kms.KeyManagementError(
          'decrypt: iv (12 bytes) required for symmetric'
        )
      const tag =
        dec.tag instanceof Uint8Array
          ? Buffer.from(dec.tag)
          : typeof dec.tag === 'string'
            ? Buffer.from(dec.tag, 'base64')
            : null
      const aad =
        dec.aad instanceof Uint8Array
          ? Buffer.from(dec.aad)
          : typeof dec.aad === 'string'
            ? Buffer.from(dec.aad)
            : undefined
      const decipher = createDecipheriv('chacha20-poly1305', symKey, iv, {
        authTagLength: 16,
      } as Parameters<typeof createDecipheriv>[3])
      if (aad)
        (decipher as unknown as { setAAD(buf: Buffer): void }).setAAD(aad)
      if (tag)
        (decipher as unknown as { setAuthTag(buf: Buffer): void }).setAuthTag(
          tag
        )
    ...
    throw new Kms.KeyManagementError(
      'decrypt: key.keyAgreement or key.privateJwk (oct) required'
    )
  }
