...
/** Adaptador KMS remoto. Delega a kms-service vía HTTP. */
export class RemoteKeyManagementService {
  ...
  public async encrypt(_agentContext: AgentContext, options: Kms.KmsEncryptOptions): Promise<Kms.KmsEncryptReturn> {
    return await this.call('/encrypt', { method: 'POST', body: JSON.stringify(options), headers: { 'content-type': 'application/json' } }) as Kms.KmsEncryptReturn
  }

  public async decrypt(_agentContext: AgentContext, options: Kms.KmsDecryptOptions): Promise<Kms.KmsDecryptReturn> {
    return await this.call('/decrypt', { method: 'POST', body: JSON.stringify(options), headers: { 'content-type': 'application/json' } }) as Kms.KmsDecryptReturn
  }
