...
//#region src/util/messageType.ts
...
function parseMessageType(messageType) {
	...
	if (!match) throw new Error(`Invalid message type: ${messageType}`);
...
function parseDidCommProtocolUri(didCommProtocolUri) {
	...
	if (!match) throw new Error(`Invalid protocol uri: ${didCommProtocolUri}`);
...
function supportsIncomingDidCommProtocolUri(incomingProtocolUri, expectedProtocolUri, { allowLegacyDidSovPrefixMismatch = true } = {}) {
	const incomingDocumentUri = allowLegacyDidSovPrefixMismatch ? replaceLegacyDidSovPrefix(incomingProtocolUri.documentUri) : incomingProtocolUri.documentUri;
	...
	return ValidateBy({
		...
		validator: {
			validate: (value, args) => {
				...
				return supportsIncomingMessageType(parseMessageType(value), expectedMessageType);
			},
			defaultMessage: buildMessage((eachPrefix) => `${eachPrefix}$property does not match the expected message type (only minor version may be lower)`, validationOptions)
		}
	}, validationOptions);
}
...
function replaceLegacyDidSovPrefix(messageType) {
	...
	if (messageType.startsWith(didSovPrefix)) return messageType.replace(didSovPrefix, didCommPrefix);
	return messageType;
}
function replaceNewDidCommPrefixWithLegacyDidSov(messageType) {
	const didSovPrefix = "did:sov:BzCbsNYhMrjHiqZDTUASHg;spec";
	const didCommPrefix = "https://didcomm.org";
	if (messageType.startsWith(didCommPrefix)) return messageType.replace(didCommPrefix, didSovPrefix);
	return messageType;
}

//#endregion
export { IsValidMessageType, canHandleMessageType, parseDidCommProtocolUri, parseMessageType, replaceLegacyDidSovPrefix, replaceLegacyDidSovPrefixOnMessage, replaceNewDidCommPrefixWithLegacyDidSovOnMessage, supportsIncomingDidCommProtocolUri, supportsIncomingMessageType };
//# sourceMappingURL=messageType.mjs.map