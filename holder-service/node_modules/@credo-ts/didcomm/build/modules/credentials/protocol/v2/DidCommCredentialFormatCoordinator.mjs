...
//#region src/modules/credentials/protocol/v2/DidCommCredentialFormatCoordinator.ts
var DidCommCredentialFormatCoordinator = class {
	/**
	...
	*
	*/
	async createProposal(agentContext, { credentialFormats, formatServices, credentialExchangeRecord, comment, goalCode, goal }) {
		...
		for (const formatService of formatServices) {
			...
			if (previewAttributes) credentialPreview = new DidCommCredentialV2Preview({ attributes: previewAttributes });
	...
	async processProposal(agentContext, { credentialExchangeRecord, message, formatServices }) {
		const didCommMessageRepository = agentContext.dependencyManager.resolve(DidCommMessageRepository);
		for (const formatService of formatServices) {
	...
	async acceptProposal(agentContext, { credentialExchangeRecord, credentialFormats, formatServices, comment, goalCode, goal }) {
		const didCommMessageRepository = agentContext.dependencyManager.resolve(DidCommMessageRepository);
		const formats = [];
		...
		credentialExchangeRecord.credentialAttributes = credentialPreview?.attributes;
		if (!credentialPreview) credentialPreview = new DidCommCredentialV2Preview({ attributes: [] });
	...
	*
	*/
	async createOffer(agentContext, { credentialFormats, formatServices, credentialExchangeRecord, comment, goalCode, goal }) {
		const didCommMessageRepository = agentContext.dependencyManager.resolve(DidCommMessageRepository);
		const formats = [];
		const offerAttachments = [];
		let credentialPreview;
	...
	async processOffer(agentContext, { credentialExchangeRecord, message, formatServices }) {
		const didCommMessageRepository = agentContext.dependencyManager.resolve(DidCommMessageRepository);
		for (const formatService of formatServices) {
	...
	async acceptOffer(agentContext, { credentialExchangeRecord, credentialFormats, formatServices, comment, goalCode, goal }) {
		const didCommMessageRepository = agentContext.dependencyManager.resolve(DidCommMessageRepository);
	...
	*
	*/
	async createRequest(agentContext, { credentialFormats, formatServices, credentialExchangeRecord, comment, goalCode, goal }) {
		const didCommMessageRepository = agentContext.dependencyManager.resolve(DidCommMessageRepository);
		const formats = [];
		const requestAttachments = [];
		for (const formatService of formatServices) {
	...
	async processRequest(agentContext, { credentialExchangeRecord, message, formatServices }) {
		const didCommMessageRepository = agentContext.dependencyManager.resolve(DidCommMessageRepository);
		for (const formatService of formatServices) {
	...
	async acceptRequest(agentContext, { credentialExchangeRecord, credentialFormats, formatServices, comment, goalCode, goal }) {
		const didCommMessageRepository = agentContext.dependencyManager.resolve(DidCommMessageRepository);
	...
	async processCredential(agentContext, { credentialExchangeRecord, message, requestMessage, formatServices }) {
		const didCommMessageRepository = agentContext.dependencyManager.resolve(DidCommMessageRepository);
		const offerMessage = await didCommMessageRepository.getAgentMessage(agentContext, {
			associatedRecordId: credentialExchangeRecord.id,
			messageClass: DidCommOfferCredentialV2Message,
...
//#endregion
export { DidCommCredentialFormatCoordinator };
//# sourceMappingURL=DidCommCredentialFormatCoordinator.mjs.map