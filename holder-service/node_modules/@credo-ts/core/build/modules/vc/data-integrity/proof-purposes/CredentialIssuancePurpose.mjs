...
import { purposes, suites } from "../libraries/jsonld-signatures.mjs";

//#region src/modules/vc/data-integrity/proof-purposes/CredentialIssuancePurpose.ts
	...
	*/
	async validate(proof, options) {
		try {
			...
			if (!issuer || issuer.length === 0) throw new Error("Credential issuer is required.");
			const issuerId = typeof issuer[0] === "string" ? issuer[0] : issuer[0].id;
			if (result.controller.id !== issuerId) throw new Error("Credential issuer must match the verification method controller.");
			return { valid: true };
		} catch (error) {
			return {
				valid: false,
...
//# sourceMappingURL=CredentialIssuancePurpose.mjs.map