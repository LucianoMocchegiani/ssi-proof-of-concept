...
function assertAllowedSigningAlgForKey(jwk, algorithm) {
	...
	if (!allowedAlgs.includes(algorithm)) {
		...
		throw new KeyManagementError(`${getJwkHumanDescription(jwk)} cannot be used with algorithm '${algorithm}' for signature creation or verification.${allowedAlgsText}`);
	}
}
function supportedSigningAlgsForKey(jwk) {
	if (jwk.kty === "EC" || jwk.kty === "OKP") switch (jwk.crv) {
		case "secp256k1": return ["ES256K"];
		case "P-256": return ["ES256"];
		case "P-384": return ["ES384"];
		case "P-521": return ["ES512"];
		case "Ed25519": return ["EdDSA", "Ed25519"];
		default: return [];
	}
	if (jwk.kty === "RSA") {
...
function createKeyTypeForSigningAlgorithm(algorithm) {
	...
	if (algorithm === "ES384") return {
		kty: "EC",
		crv: "P-384"
	};
	if (algorithm === "ES512") return {
		kty: "EC",
		crv: "P-521"
	};
	if (algorithm === "ES256K") return {
		kty: "EC",
		crv: "secp256k1"
	};
	if (algorithm === "EdDSA" || algorithm === "Ed25519") return {
		kty: "OKP",
		crv: "Ed25519"
	};
	...
	if (algorithm === "HS384") return {
		kty: "oct",
		algorithm: "hmac",
		length: 384
	};
	if (algorithm === "HS512") return {
		kty: "oct",
		algorithm: "hmac",
		length: 512
	};
	if (algorithm === "PS256" || algorithm === "RS256") return {
		kty: "RSA",
		modulusLength: 2048
	};
	if (algorithm === "PS384" || algorithm === "RS384") return {
		kty: "RSA",
		modulusLength: 3072
	};
	if (algorithm === "PS512" || algorithm === "RS512") return {
		kty: "RSA",
		modulusLength: 4096
	};
...
//#endregion
export { allowedSigningAlgsForSigningKey, assertAllowedSigningAlgForKey, createKeyTypeForSigningAlgorithm, supportedSigningAlgsForKey };
