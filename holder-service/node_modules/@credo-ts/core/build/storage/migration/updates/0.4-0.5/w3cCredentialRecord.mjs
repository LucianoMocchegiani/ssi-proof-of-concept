...
import { W3cJsonLdCredentialService } from "../../../../modules/vc/data-integrity/W3cJsonLdCredentialService.mjs";
...
//#region src/storage/migration/updates/0.4-0.5/w3cCredentialRecord.ts
/**
...
async function fixIncorrectExpandedTypesWithAskarStorage(agent, w3cCredentialRecord) {
	...
	if (!(w3cCredentialRecord.getTag("expandedTypes") ?? []).some((type) => type === "https")) return;
	...
	try {
		const newExpandedTypes = await w3cJsonLdCredentialService.getExpandedTypesForCredential(agent.context, firstCredential);
...
//#endregion
export { migrateW3cCredentialRecordToV0_5 };
