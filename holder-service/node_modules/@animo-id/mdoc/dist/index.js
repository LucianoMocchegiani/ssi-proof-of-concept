...
__export(src_exports, {
  ...
  limitDisclosureToDeviceRequestNameSpaces: () => limitDisclosureToDeviceRequestNameSpaces,
...
var _ItemsRequest = class _ItemsRequest {
  ...
  get data() {
    if (!__privateGet(this, _dataRecord)) {
      const decoded = cborDecode(__privateGet(this, _dataItem2).buffer, {
        tagUint8Array: false,
        useRecords: true,
        mapsAsObjects: true
      });
      __privateSet(this, _dataRecord, {
        ...decoded,
        nameSpaces: new Map(
  ...
  prepare() {
    const nameSpaces = new Map(
      Array.from(this.issuerSigned.nameSpaces.entries()).map(
        ([nameSpace, items]) => [nameSpace, items.map((item) => item.dataItem)]
      )
    );
...
var namespaceToArray = (entries) => entries.map((di) => new IssuerSignedItem(di));
var mapIssuerNameSpaces = (namespace) => new Map(Array.from(namespace.entries()).map(([nameSpace, entries]) => [nameSpace, namespaceToArray(entries)]));
...
var DeviceResponse = class _DeviceResponse {
  ...
  usingSessionTranscriptCallback(sessionTranscriptCallback) {
    if (this.sessionTranscriptBytes || this.sessionTranscriptCallback) {
      throw new Error(
        "A session transcript has already been set, either with .usingSessionTranscriptForOID4VP, .usingSessionTranscriptForWebAPI or .usingSessionTranscriptBytes"
      );
  ...
  usingSessionTranscriptForOID4VP(input) {
    this.usingSessionTranscriptCallback(
      (context) => _DeviceResponse.calculateSessionTranscriptBytesForOID4VP({ ...input, context })
    );
    return this;
  }
  static async calculateSessionTranscriptBytesForOID4VP(input) {
  ...
  static async calculateSessionTranscriptBytesForOID4VPDCApi(input) {
  ...
  usingSessionTranscriptForForOID4VPDCApi(input) {
    this.usingSessionTranscriptCallback(
      (context) => _DeviceResponse.calculateSessionTranscriptBytesForOID4VPDCApi({ ...input, context })
    );
    return this;
  }
  /**
  ...
  usingSessionTranscriptForWebAPI(input) {
    this.usingSessionTranscriptCallback(
      (context) => _DeviceResponse.calculateSessionTranscriptBytesForWebApi({ ...input, context })
    );
    return this;
  }
  static async calculateSessionTranscriptBytesForWebApi(input) {
  ...
  async sign(ctx) {
    ...
    if (!this.sessionTranscriptBytes && this.sessionTranscriptCallback) {
      ...
      this.sessionTranscriptCallback = void 0;
    }
  ...
  async getDeviceSigned(docType, sessionTranscriptBytes, ctx) {
...
var Document = class {
  constructor(doc, ctx) {
    __privateAdd(this, _issuerNameSpaces, /* @__PURE__ */ new Map());
  ...
  addIssuerNameSpace(namespace, values) {
    const namespaceRecord = __privateGet(this, _issuerNameSpaces).get(namespace) ?? [];
    ...
    __privateGet(this, _issuerNameSpaces).set(namespace, namespaceRecord);
    return this;
  }
  /**
   ...
   */
  getIssuerNameSpace(namespace) {
    const nameSpace = __privateGet(this, _issuerNameSpaces).get(namespace);
    if (!nameSpace) return void 0;
  ...
  addValidityInfo(info = {}) {
    ...
    if (info.expectedUpdate) {
      __privateGet(this, _validityInfo).expectedUpdate = info.expectedUpdate;
    }
    return this;
  }
  /**
  ...
  async sign(params, ctx) {
    ...
    const valueDigests = new Map(
      await Promise.all(
        Array.from(__privateGet(this, _issuerNameSpaces).entries()).map(async ([namespace, items]) => {
...
var Verifier = class {
        ...
        if (ns === MDL_NAMESPACE) {
          ...
          if (!certificateData.issuerName) {
            ...
            onCheck({
              ...
              check: "The 'issuing_country' if present must match the 'countryName' in the subject field within the DS certificate",
              reason: invalidCountry ? `The 'issuing_country' (${invalidCountry.ev.elementValue}) must match the 'countryName' (${issuerAuth.getIssuingCountry(ctx)}) in the subject field within the issuer certificate` : void 0
            });
            ...
            onCheck({
              ...
              reason: invalidJurisdiction ? `The 'issuing_jurisdiction' (${invalidJurisdiction.ev.elementValue}) must match the 'stateOrProvinceName' (${issuerAuth.getIssuingStateOrProvince(ctx)}) in the subject field within the issuer certificate` : void 0
            });
          }
        }
      })
  ...
  async getDiagnosticInformation(encodedDeviceResponse, options, ctx) {
    ...
    const deviceAttributes = document instanceof DeviceSignedDocument ? Array.from(document.deviceSigned.nameSpaces.entries()).flatMap(([ns, items]) => {
